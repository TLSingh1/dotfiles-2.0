...
require("ultimate-autopair").setup({
  ...
  cmap = true,         --cmap stands for cmd-line map whether to allow any cmd-line map
  pair_map = true,     --whether to allow pair insert map
  pair_cmap = true,    --whether to allow pair cmd-line map
  ...
  bs = {               -- *ultimate-autopair-map-backspace-config*
    ...
    space = true,      --false, true or 'balance' ( |foo ) > bs > (|foo)
    --balance:
    --  Will prioritize balanced spaces
    --  ( |foo  ) > bs > ( |foo )
    indent_ignore = false,                                      --(\n\t|\n) > bs > (|)
    single_delete = false,                                      -- <!--|--> > bs > <!-|
    conf = {},                                                  --contains extension config
  ...
  cr = {                                                        -- *ultimate-autopair-map-newline-config*
    ...
    autoclose = false,                                          --(| > cr > (\n|\n)
    conf = { cond = function(fn) return not fn.in_lisp() end }, --contains extension config
    multi = false,                                              --use multiple configs (|ultimate-autopair-map-multi-config|)
  },
  space = {                                                     -- *ultimate-autopair-map-space-config*
    ...
    _check_box_ft2 = { 'norg' },                                --may be removed + [|] > space > + [ ]
    conf = {},                                                  --contains extension config
    multi = false,                                              --use multiple configs (|ultimate-autopair-map-multi-config|)
  },
  space2 = {                                                    -- *ultimate-autopair-map-space2-config*
    enable = false,
    match = [[\k]],                                             --what character activate
    conf = {},                                                  --contains extension config
    multi = false,                                              --use multiple configs (|ultimate-autopair-map-multi-config|)
  },
  fastwarp = {                                                  -- *ultimate-autopair-map-fastwarp-config*
    ...
    faster = false, --only enables jump over pair, goto end/next line useful for the situation of: {|}M.foo('bar') > {M.foo('bar')|}
    conf = {},      --contains extension config
    multi = false,  --use multiple configs (|ultimate-autopair-map-multi-config|)
  },
